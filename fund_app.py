...
with st.expander("➕ 添加/修改持仓"):
    ...
    # 使用session_state控制输入框清空
    if 'input_code' not in st.session_state:
        st.session_state.input_code = ""
    if 'input_shares' not in st.session_state:
        st.session_state.input_shares = 0.0
    ...
    with c1:
        # 基金代码输入（点击即清空）
        new_code = st.text_input("代码", value=st.session_state.input_code, max_chars=6, key="code_input",
        ...
        def clear_code():
            st.session_state.input_code = ""
        if new_code != st.session_state.input_code:
            st.session_state.input_code = new_code
    
    with c2:
        # 份额输入（点击即清空）
        new_shares = st.number_input("份额", value=st.session_state.input_shares, min_value=0.0, format="%.2f",
                                    key="shares_input", label_visibility="collapsed")
        st.caption("持有份额")
        if new_shares != st.session_state.input_shares:
            st.session_state.input_shares = new_shares
    
    with c3:
        # 成本输入（点击即清空）
        new_cost = st.number_input("成本", value=st.session_state.input_cost, min_value=0.0, format="%.4f",
                                  key="cost_input", step=0.0001, label_visibility="collapsed")
        st.caption("成本价")
        if new_cost != st.session_state.input_cost:
            st.session_state.input_cost = new_cost
