...
def get_fund_data(codes):
    data_list=[]
    try:
        df=ak.fund_em_valuation()
        for code in codes:
            row=df[df['基金代码']==code]
            if not row.empty:
                g=str(row['估算增长率'].values[0]).replace('%','')
                data_list.append({
                    'code':code,
                    'name':row['基金名称'].values[0],
                    'nav':float(row['估算净值'].values[0]),
                    'growth':float(g)if g not in['nan','--']else 0,
                    'last':float(row['单位净值'].values[0])
                })
