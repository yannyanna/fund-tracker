...
def fetch_fund_realtime(code):
    try:
        ...
        with urllib.request.urlopen(req, timeout=3, context=ssl_ctx) as res:
            ...
            return {
                "name": data['name'], 
                "last_nav": float(data['dwjz']), # 昨净
                "est_nav": float(data['gsz']),  # 当前估值/净值
                "rate": float(data['gszzl']),   # 涨幅
                "time": data['gztime'],
                "is_closed": data['jzrq'] == data['gztime'][:10]
            }
    except: return None
...
for h in db["holdings"]:
    ...
    if rt:
        ...
        # --- 核心修复：今日收益 = 份额 * (当前价 - 昨净值) ---
        # 这种算法不依赖涨幅百分比，不会受接口跳变影响
        current_price = rt['est_nav']
        m_val = shares * current_price
        d_profit = shares * (current_price - rt['last_nav']) 
...
for f in fund_results:
    ...
    <div class="fund-card">
        ...
        <div class="fund-grid">
            ...
            <div class="fund-item"><div class="fund-label">今日收益</div><div class="fund-value {c_cls}">{f['d_profit']:+.2f}</div></div>
            <div class="fund-item"><div class="fund-label">累计盈亏</div><div class="fund-value {t_cls}">{f['h_profit']:+.2f}</div></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with st.expander("⚙️ 持仓管理", expanded=st.session_state.get('admin_expanded', True)):
    with st.form("fund_form_v3", clear_on_submit=False):
        c1, c2 = st.columns([1, 4])
        with c1: st.markdown('<div class="input-label">代码</div>', unsafe_allow_html=True)
        with c2: m_code = st.text_input("code_v3", max_chars=6, label_visibility="collapsed")
        c3, c4 = st.columns([1, 4])
        with c3: st.markdown('<div class="input-label">份额</div>', unsafe_allow_html=True)
        with c4: m_shares = st.number_input("share_v3", value=None, label_visibility="collapsed")
        c5, c6 = st.columns([1, 4])
        with c5: st.markdown('<div class="input-label">成本</div>', unsafe_allow_html=True)
        with c6: m_cost = st.number_input("cost_v3", value=None, format="%.4f", label_visibility="collapsed")
