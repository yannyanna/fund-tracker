...
def get_fund_data(codes):
    ...
    for code in codes:
        try:
            ...
            # 计算涨跌幅
            try:
                growth=float(d['gszzl']) if d.get('gszzl') else 0
            except:
                growth=0
            ...
            data_list.append({
                ...
                'nav':float(d.get('gsz',0)),  # 估算净值
                'growth':growth,  # 估算增长率
                'last':float(d.get('dwjz',0))  # 单位净值
            })
...
<div class="update-time">
    更新于:{datetime.now().strftime('%H:%M:%S')}|30秒后自动刷新<br><small>估值仅供参考，实际以基金公司公布为准</small>
</div>
""",unsafe_allow_html=True)
