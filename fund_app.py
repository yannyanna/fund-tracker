...
st.set_page_config(page_title="资产管理最终修复版", layout="wide", initial_sidebar_state="expanded")
...
for h in db["holdings"]:
    ...
    if f:
        ...
        total_p = shares * (f['price'] - cost) # 修复这里的变量冲突
        m_val = shares * f['price']
        
        total_market_val += m_val
        total_day_profit += day_p
        fund_results.append({**h, **f, "day_p": day_p, "total_p": total_p, "m_val": m_val})
...
with st.expander("⚙️ 管理我的持仓", expanded=st.session_state.get('admin_exp', True)):
    with st.form("fund_form_v4"):
        c1, c2 = st.columns([1, 4])
        with c1: st.markdown('<div class="input-label">代码</div>', unsafe_allow_html=True)
        with c2: m_code = st.text_input("code_input", max_chars=6, label_visibility="collapsed")
        
        c3, c4 = st.columns([1, 4])
        with c3: st.markdown('<div class="input-label">份额</div>', unsafe_allow_html=True)
        # 将 value 设为 None，实现进入时清空原有数据
        with c4: m_shares = st.number_input("shares_input", value=None, format="%.2f", label_visibility="collapsed")
        
        c5, c6 = st.columns([1, 4])
        with c5: st.markdown('<div class="input-label">成本</div>', unsafe_allow_html=True)
        # 将 value 设为 None
        with c6: m_cost = st.number_input("cost_input", value=None, format="%.4f", label_visibility="collapsed")
